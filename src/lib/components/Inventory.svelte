<script lang="ts">
  ...
  onMount(() => {
    pb.collection("inventory_items").subscribe("*", () => {
    ...
    const find = async () => {
      ...
      const filtered = await pb
        ...
        .getList(pageNumber, 5, {
          filter: nameSearch
            ? `id ~ "%${nameSearch}%" || ingredient.name ~ "%${nameSearch}%"`
            : "",
          sort: sortBy,
          expand: "ingredient",
        });
  ...
  export let crud: "entry" | "exit" | false = false;
  ...
  async function save() {
    if (toEdit.id) {
      await pb.collection("inventory_items").update(toEdit.id, {
        ...toEdit,
        type: crud,
      });
    } else {
    ...
    const data = await pb.collection("ingredients").getOne(toEdit.ingredient);
    await pb.collection("ingredients").update(data.id, {
      quantity: Math.max(
        0,
        crud == "entry"
          ? +data.quantity + +toEdit.quantity
          : +data.quantity - +toEdit.quantity
      ),
    });
    cancelCrud();
  ...
  async function del() {
    await pb.collection("inventory_items").delete(toEdit.id);
  }
</script>
    ...
    >
      <div class="flex space-x-8 justify-between">
        <div class="flex space-x-2 items-center">
          ...
          >
            {toEdit.id ? "Editar" : "Crear"}
            {crud == "entry" ? "entrada" : "salida"}
          </h4>
        </div>
      ...
      <div class="flex flex-col h-4/10">
        <Ingredients selectable bind:selected={toEdit.ingredient} />
      </div>
...
>
  <div class="flex space-x-2 w-full justify-between items-center">
    <input
      ...
      placeholder="Busca por id o nombre del ingrediente"
    />
    <Submenu>
      <button
      ...
      >
        <div class="flex flex-col space-y-3 pb-3">
          <p class="font-bold">Nombre del ingrediente</p>
          <label class="flex font-normal space-x-2 text-xs">
            <input type="radio" value="ingredient.name" bind:group={sortBy} />
  ...
  {:else}
        ...
        >
          ...
          >
            <tr>
              ...
              <th scope="col" class="py-3 px-6"> Ingrediente </th>
            ...
            {#each data.items as c, idx}
              ...
              >
                ...
                <td>
                  <div class="flex space-x-2 w-full py-4 px-6 items-center">
                    <p class="font-bold text-xs">
                      {c.quantity * c.expand.ingredient.weight}{c.expand
                        .ingredient.weight_type}
                    </p>
                  </div>
                </td>
                <td>
    ...
    <div class="flex w-full items-center justify-between">
      <span class="font-bold text-xs leading-0">
        {data?.totalItems || 0} elementos encontrados
      </span>
      <div class="flex space-x-2 items-center">
        <button
          title="Previous page"
